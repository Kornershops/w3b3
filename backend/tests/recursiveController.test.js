...
jest.mock('../dist/services/recursiveYieldService', () => ({
...
describe('RecursiveController', () => {
  ...
  test('passes validated input to the service', async () => {
    ...
    recursiveYieldService.simulateLoop.mockResolvedValue(simulation);
  ...
  test('maps strategy-limit failures to HTTP 400', async () => {
    recursiveYieldService.simulateLoop.mockRejectedValue(new Error('Requested leverage exceeds strategy maximum'));
